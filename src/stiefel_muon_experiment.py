    ...
    *,
    ...
    for name, scheduler in schedulers.items():
        ...
        if scheduler_t_max is not None:
            # Keep the checkpoint's current learning rate and epoch, then
            # extend only the remaining cosine tail.  Without this override,
            # extending beyond the original horizon makes CosineAnnealingLR
            # rise again as a periodic schedule.
...
def _historical_baseline_records(root: Path, optimizer: str) -> list[dict]:
    """Put the selected tuned baseline on its actual optimizer-step timeline."""
...
def _write_constrained_muon_comparison_plots(
    ...
    for optimizer in ("adamw", "muon", "muown"):
        ...
        if metric.exists() and result.exists():
            traces.append((selected_baseline_label(optimizer), _historical_baseline_records(root, optimizer)))
    ...
    for output, key, xlabel in (
        ...
        axis.set_title(f"{display_name} versus selected GPT-2 baselines")
        axis.grid(alpha=0.2)
        axis.legend()
        figure.tight_layout()
