"""Regenerate historical GPT comparison plots after a baseline update.

The candidate metrics are immutable experiment records.  This utility only
refreshes the selected common baseline traces from their current canonical files.
"""
...
NON_FINAL_PLOT_MARKERS = ("_tune", "_pilot", "_probe", "_profile", "_qual")
...
def _candidate_paths(root: Path) -> list[tuple[str, Path]]:
    ...
    for output in sorted(results.glob("*_metric_steps.png")):
        ...
        if not _is_final_candidate(stem):
            continue
...
def _is_final_candidate(stem: str) -> bool:
    """Exclude retained tuning-only images from the baseline refresh."""
    lowered = stem.lower()
    return not (
        any(marker in lowered for marker in NON_FINAL_PLOT_MARKERS)
        or lowered.startswith("hybrid_stiefel_muon_equal_time_")
    )
...
def _write_plot(
    ...
    *,
    ...
    muown_records: list[dict],
    coordinate: str,
) -> None:
    x_label = "Optimizer step" if coordinate == "step" else "Wall-clock time (hours)"
    figure, axis = plot.subplots(figsize=(9, 5))
    for label, records in (
        (selected_baseline_label("adamw"), adamw_records),
        (selected_baseline_label("muon"), muon_records),
        (selected_baseline_label("muown"), muown_records),
    ...
    axis.set(
        ...
        title=f"{_display_name(candidate_name)} versus selected GPT-2 baselines",
    )
    axis.grid(alpha=0.2)
    axis.legend()
    figure.tight_layout()
...
def regenerate(root: Path, *, dry_run: bool = False) -> list[Path]:
    ...
    muown_records = _baseline_records(root, "muown")
    outputs: list[Path] = []
