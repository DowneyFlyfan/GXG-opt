...
MUOWN_FINAL_RUN = TuningRun("Muown lr=0.005 (wd=0)", "lr0005_b8_a6_final", "muown")
FINAL_BASELINE_RUNS = LITERATURE_TUNING_RUNS + (MUOWN_FINAL_RUN,)
...
def select_best_final_runs(
    root: Path, runs: tuple[TuningRun, ...] = FINAL_BASELINE_RUNS
...
def write_final_baseline_plots(
    ...
    """Write final plots from the selected completed GPT baselines."""
    selected = select_best_final_runs(root, runs)
    if set(selected) != {"adamw", "muon", "muown"}:
        return None
    ...
    adamw, muon, muown = selected["adamw"], selected["muon"], selected["muown"]
...
def write_selected_baseline_alias_plots(root: Path) -> tuple[Path, Path] | None:
    """Refresh the legacy GPT baseline filenames from the selected winners."""
    selected = select_best_final_runs(root)
    if set(selected) != {"adamw", "muon", "muown"}:
        return None
    # Resolve the exact run corresponding to each selected trace, rather than
    # treating the legacy canonical files as the final tuned comparison.
    metric_paths = {}
    for optimizer, trace in selected.items():
        run = next(run for run in FINAL_BASELINE_RUNS if run.label == trace.label)
        metric_paths[optimizer] = _paths(root, run)[0]
    output_root = root / "results" / "nlp"
    output_root.mkdir(parents=True, exist_ok=True)
    outputs = (
        output_root / f"{TASK}_metric_steps.png",
        output_root / f"{TASK}_metric_time.png",
    )
    runtimes = {
        "AdamW": selected["adamw"].seconds,
        "Muon": selected["muon"].seconds,
        "Muown": selected["muown"].seconds,
    }
    write_metric_plot(
        metric_paths["adamw"],
        metric_paths["muon"],
