...
def trial_artifact_paths(
    ...
    *,
    ...
    """Place durable experiment outputs outside the cache and checkpoints inside it."""
    stem = task.identifier if run_label is None else f"{task.identifier}__{run_label}"
    result_root = root / "results" / task.domain
    checkpoint_root = root / ".cache" / task.domain / "checkpoints"
    return TrialArtifactPaths(
        ...
        checkpoint=checkpoint_root / f"{stem}__{optimizer_name}.checkpoint.pt",
...
def run_trial(
    ...
    resolved_learning_rate = (
        task.adamw_lr if optimizer_name == "adamw" else task.muon_lr
    ) if learning_rate is None else learning_rate
    if resolved_learning_rate <= 0:
        raise ValueError("learning_rate must be positive")
    resolved_weight_decay = task.weight_decay if weight_decay is None else weight_decay
