"""Comparison plots for the Effective-Rank-Third GPT2-12x512 optimizer."""
...
def _candidate_records(root: Path, label: str) -> list[dict]:
    stem = f"nlp_gpt_12x512__{label}__effective_rank_third"
...
def write_effective_rank_third_comparison_plots(
    root: Path, *, label: str
    ...
    traces = [
        ...
        ("Effective-Rank-Third", _candidate_records(root, label)),
    ]
    ...
    outputs = (
        output_root / f"effective_rank_third_{label}_metric_steps.png",
        output_root / f"effective_rank_third_{label}_metric_time.png",
    )
    for output, key, xlabel in (
        ...
        axis.set(
            ...
            title="Effective-Rank-Third versus tuned GPT-2 baselines",
        )
        axis.grid(alpha=0.2)
        axis.legend()
        figure.tight_layout()
