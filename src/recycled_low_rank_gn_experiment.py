...
def write_kron_rpcg_comparison_plots(
    ...
    *,
    ...
    for label, optimizer, trace_label in (
        ...
        path = (
            selected_baseline_paths(root, optimizer)[0]
            if optimizer in {"adamw", "muon", "muown"} and trace_label is None
            else artifact_paths(root, optimizer, run_label=trace_label).metric
        )
        if path.exists():
            traces.append(
