...
def _comparison_records(
    ...
    if optimizer_name in {"adamw", "muon", "muown"} and run_label is None:
        metric_path, result_path = selected_baseline_paths(root, optimizer_name)
    else:
        paths = artifact_paths(root, optimizer_name, run_label=run_label)
        metric_path, result_path = paths.metric, paths.result
    records = _read_metrics(metric_path)
    if not records or "step" in records[0]:
        return records
    result = json.loads(result_path.read_text())
    accumulation = int(result.get("gradient_accumulation", 1))
    steps_per_epoch = (12_207 + accumulation - 1) // accumulation
    return [
        {
            **record,
...
def write_gn_comparison_plots(
    ...
    *,
    ...
    names = [
        (selected_baseline_label("adamw"), "adamw", None),
        (selected_baseline_label("muon"), "muon", None),
        (selected_baseline_label("muown"), "muown", None),
    ]
    ...
    for label, name, run_label in names:
        path = (
            selected_baseline_paths(root, name)[0]
            if name in {"adamw", "muon", "muown"} and run_label is None
            else artifact_paths(root, name, run_label=run_label).metric
        )
        if path.exists():
            traces.append(
