...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run formal AdamW, Muon, and Muown baselines")
    parser.add_argument("--task", choices=[task.identifier for task in FORMAL_TASKS])
    parser.add_argument("--optimizer", choices=("adamw", "muon", "muown"))
    parser.add_argument("--workers", type=int, default=4)
    args = parser.parse_args()
    root = Path(__file__).resolve().parents[1]
    tasks = [task for task in FORMAL_TASKS if args.task is None or task.identifier == args.task]
    optimizers = (args.optimizer,) if args.optimizer else ("adamw", "muon", "muown")
    for task in tasks:
